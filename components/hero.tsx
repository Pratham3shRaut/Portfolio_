...
export function Hero() {
  ...
  return <section onPointerMove={e => { mx.set((e.clientX / innerWidth - .5) * 36); my.set((e.clientY / innerHeight - .5) * 24); }} className="relative flex min-h-[94svh] items-end overflow-hidden pb-16 pt-36 md:pb-20">
    <div className="noise pointer-events-none absolute inset-0 opacity-[.025]"/>
    <motion.div style={{ x, y }} className="pointer-events-none absolute right-[-8rem] top-[18%] h-[34rem] w-[34rem] rounded-full border border-signal/40 opacity-70 md:right-[4%]">
    ...
    <div className="frame relative z-10">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1, duration: .7 }} className="mb-8 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[.2em]"><span className="relative h-2 w-2 rounded-full bg-[#21a179] before:absolute before:inset-[-4px] before:animate-ping before:rounded-full before:border before:border-[#21a179]"/>Full Stack Developer · Pune</motion.div>
      ...
      <div className="mt-10 flex flex-col justify-between gap-8 md:flex-row md:items-end">
        <div className="flex items-center gap-5">
          {hasPortrait && (
            ...
            <img src="/hero.jpg" alt="Prathamesh Raut" width={72} height={72} onError={() => setHasPortrait(false)} className="h-[4.5rem] w-[4.5rem] shrink-0 rounded-full border hairline object-cover shadow-lg" />
          )}
        ...
        <MagneticLink href="#work">Explore selected work</MagneticLink>
      </div>
    </div>
